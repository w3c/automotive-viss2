...
The authorization server used in this project may not meet the security robustness required in a real life deployment. Initially, it may simply have fixed yes/no response that may be toggled during testing. 
...
The encoding is currently only possible to activate over the WebSocket transport protocol, but it would lso be possible to invoke over the HTTP protocol. 
...
The mchanism of using the sub-protocol parameter to invoke encoding scheme can easily be extended to use other compression schemes.
